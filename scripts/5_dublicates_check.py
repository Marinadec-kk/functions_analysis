...
class HybridCollisionAnalyzerApp:
            ...
            while True:
                ...
                if command == "status":
                    ...
                    self.eta_label.config(
                        text="Примерное время ожидания: вычисление..."
                    )
                    ...
                    self.progress_label.config(
                        text=f"Проверено пар: {current} / {total}"
                    )

                    # --- НОВЫЙ БЛОК: Расчет и обновление ETA ---
                    if (
                        self._timer_running and current > 5 and total > 0
                    ):  # Начинаем расчет после 5 пар для стабильности
                        ...
                        self.eta_label.config(
                            text=f"Примерное время ожидания: {eta_str}"
                        )
                    # --- КОНЕЦ НОВОГО БЛОКА ---
    ...
    def _build_ui(self):
        ...
        self._create_entry_row(
            self.online_frame, "Количество воркеров:", self.ai_workers_var
        )
        ...
        ttk.Label(
            self.local_frame,
            text=f"Будет создано {WORKERS_PER_LOCAL_SERVER} воркера на каждый сервер.",
        ).pack(anchor="w", padx=5)
        ttk.Label(
            self.local_frame, text="Адреса серверов LM Studio (каждый с новой строки):"
        ...
        self._create_entry_row(
            self.local_frame, "Имя модели (локально):", self.local_model_var
        )
        ...
        ttk.Checkbutton(
            common_ai_frame,
            text="JSON режим (для совместимых моделей)",
        ...
        self.start_btn = ttk.Button(
            btn_row, text="Старт анализ", command=self._on_start, style="Accent.TButton"
        )
        ...
        self.save_btn = ttk.Button(
            btn_row, text="Сохранить отчет", command=self.save_file, state=tk.DISABLED
        )
    ...
    def _on_start(self):
        ...
        try:
            ...
            if config["verification_mode"] == "Онлайн":
                ...
                if not config["ai_chat_model"]:
                    raise ValueError("В режиме 'Онлайн' укажите имя модели.")
                if config["ai_workers"] <= 0:
                    raise ValueError("Количество воркеров должно быть > 0.")
            else:  # Локальная
                local_servers = (
                ...
                if not config["local_chat_model"]:
                    raise ValueError(
                        "В режиме 'Локальная' необходимо указать имя модели."
                    )
        ...
        self.log(
            f"Используемые настройки: {json.dumps(config, indent=2, ensure_ascii=False)}",
            to_terminal=True,
        )
        self.reset_progress(0)
    ...
    def _worker_main(self, config: dict):
        ...
        try:
            ...
            self.set_status(
                f"Чтение файла: {os.path.basename(config['input_file'])}..."
            )
            self.log(
            ...
            if config["verification_mode"] == "Онлайн":
                ...
                self.log(
                    f"Режим 'Онлайн'. Модель: {model_name}. Воркеров: {config['ai_workers']}.",
                    to_terminal=True,
                )
            else:  # Локальная
                ...
                for url in config["local_servers"]:
                    try:
                        ...
                        self.log(
                            f"Подготовлен клиент для сервера: {url}", to_terminal=True
                        )
                    except Exception as e:
                        self.log(
                            f"Ошибка создания клиента для {url}: {e}", to_terminal=True
                        )

                if not llm_clients:
                    raise ConnectionError(
                        "Не удалось создать ни одного клиента для локальных серверов. Проверьте адреса."
                    )

                model_name = config["local_chat_model"]
                total_workers = len(llm_clients) * WORKERS_PER_LOCAL_SERVER
                self.log(
                    f"Режим 'Локальная'. Модель: {model_name}. Серверов: {len(llm_clients)}. Всего воркеров: {total_workers}.",
                    to_terminal=True,
                )
            ...
            for _, row in self.df[
                self.df["CollisionWith"].str.strip() != ""
            ].iterrows():
                row_id = row["ID"]
                for c_id in re.split(r"[;, ]+", row["CollisionWith"]):
                    ...
                    if clean_id not in valid_ids:
                        self.log(
                            f"ПРЕДУПРЕЖДЕНИЕ: В строке ID {row_id} ссылка на несуществующий ID: '{clean_id}'. Пара пропущена.",
                            to_terminal=True,
                        )
                        continue
                    ...
                    if clean_id == row_id:
                        self.log(
                            f"ПРЕДУПРЕЖДЕНИЕ: В строке ID {row_id} ссылка на саму себя. Пара пропущена.",
                            to_terminal=True,
                        )
                        continue
            ...
            if not tasks:
                self.ui_queue.put(
                    ("worker_done", "Корректных коллизий для анализа не найдено.")
                )
                return

            self.log(
                f"Найдено {len(tasks)} уникальных и корректных пар для AI-верификации.",
                to_terminal=True,
            )
            ...
            total_false = sum(
                1
                for res in self.results_cache.values()
                if res and res.get("verdict") == "FALSE"
            )
    ...
    def _verification_worker(
        ...
        while not task_queue.empty():
            ...
            try:
                ...
                self.log(
                    f"[{worker_name}] Взял в работу пару ID: ({id1}, {id2})",
                    to_terminal=True,
                )
                pair_key = tuple(sorted((id1, id2)))
                row1, row2 = self.df_indexed.loc[id1], self.df_indexed.loc[id2]
                verdict_json = self._get_llm_verdict(
                    client, model_name, row1, row2, config
                )
                ...
                self.log(
                    f"Критическая ошибка в воркере [{worker_name}]: {e}",
                    to_terminal=True,
                )
                break
            finally:
        ...
        self.log(
            f"[{worker_name}] Завершил работу, задачи в очереди закончились.",
            to_terminal=True,
        )

    def _get_llm_verdict(
        self, client: OpenAI, model: str, row1: pd.Series, row2: pd.Series, config: dict
    ) -> Dict:
        """Отправляет запрос к модели AI и возвращает JSON-ответ."""
        id1, id2 = row1.name, row2.name
        user_prompt = (
            f"Название ГО1: {row1['Исполняющий ГО']}\nФункция ГО1: {row1['FunctionText']}\n\n"
        ...
        for i in range(3):  # Механизм повтора
            ...
            try:
                ...
                self.log(
                    f"ОТВЕТ от AI для пары ({id1}, {id2}):\n---\n{content}\n---",
                    to_terminal=True,
                )
                ...
                if match:
                    ...
                    if "verdict" in response_json:
                        self.log(
                            f"ИТОГ для пары ({id1}, {id2}): Вердикт '{response_json['verdict']}'",
                            to_terminal=True,
                        )
                ...
                self.log(
                    f"Ошибка сети/лимитов/таймаута для пары ({id1}, {id2}): {type(e).__name__}. Попытка {i + 1}/3. Повтор через 2 сек...",
                    to_terminal=True,
                )
                time.sleep(2)
                ...
                self.log(
                    f"Ошибка API при обработке пары ({id1}, {id2}): {e}",
                    to_terminal=True,
                )
                pass
        self.log(
            f"ИТОГ для пары ({id1}, {id2}): Не удалось получить вердикт после 3 попыток. Вердикт: ERROR",
            to_terminal=True,
        )
    ...
    def update_dataframe_with_results(self, confirmed_pairs: Set):
        ...
        self.log(
            "Обновление столбцов 'CollisionWith' и 'Anomaly' в таблице.",
            to_terminal=True,
        )
        ...
        self.df["CollisionWith"] = self.df["ID"].apply(
            lambda i: "; ".join(sorted(confirmed_dict.get(i, [])))
        )
        self.df["Anomaly"] = self.df["ID"].apply(
            lambda i: "Collision" if i in confirmed_dict else ""
        )
