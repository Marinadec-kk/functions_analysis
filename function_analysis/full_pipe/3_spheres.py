    ...
    if file_path_lower.endswith(".xlsx"):
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Не удалось прочитать CSV '{file_path}'. Проверьте кодировку (UTF-8) и разделитель. Ошибка: {e}")
    else:
        raise ValueError(f"Неподдерживаемый формат файла: {file_path}. Используйте .xlsx или .csv.")
...
def require_cols(df: pd.DataFrame, cols: list, label: str):
    ...
    if miss:
        raise ValueError(f"В файле '{label}' отсутствуют необходимые колонки: {', '.join(miss)}")
...
def format_spheres_for_prompt(sphere_rows: list) -> str:
    ...
    for item in sphere_rows:
        ...
        blocks.append(f"Название сферы: {nm}\nОписание: {ds}\nВиды деятельности: {acts}")
    return "\n---\n".join(blocks)

def build_prompt(system_prompt: str, gov_body: str, function_text: str, spheres_text_filtered: str) -> List[Dict[str, str]]:
...
def parse_name_from_model(content: str) -> str:
    ...
    if "NO_MATCH" in s.upper(): return "NO_MATCH"
...
async def async_get_embedding_batch(client: openai.AsyncOpenAI, model: str, texts: List[str], semaphore: asyncio.Semaphore) -> Optional[List[List[float]]]:
    """Асинхронно получает эмбеддинги для списка текстов."""
    try:
        async with semaphore:
            if stop_event_async.is_set(): return None
            valid_texts = [t if t and t.strip() else " " for t in texts]
            resp = await client.embeddings.create(model=model, input=valid_texts, timeout=180.0)
            return [item.embedding for item in resp.data]
    except Exception as e:
        logging.error(f"Ошибка получения эмбеддингов: {e}")
        return None

async def _call_api_with_backoff(client: openai.AsyncOpenAI, model: str, messages: List[dict], temperature: float, semaphore: asyncio.Semaphore, max_tokens: int, max_retries: int, run_config: dict):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            logging.warning(f"Попытка API {attempt + 1}/{max_retries} не удалась. Ошибка статуса API: {error_details}. Повтор...")
        except Exception as e:
            last_exc = e
            logging.warning(f"Попытка API {attempt + 1}/{max_retries} не удалась. Общая ошибка: {e}. Повтор...")
...
async def classify_one_async(client: openai.AsyncOpenAI, semaphore: asyncio.Semaphore, run_config: dict, func_item: dict, spheres_prompt_filtered: str, valid_names_exact: set, valid_names_norm_map: dict) -> dict:
    ...
    messages = build_prompt(run_config['system_prompt'], gov_body, func_text, spheres_prompt_filtered)

    content = await _call_api_with_backoff(
        client, run_config['model'], messages, run_config['temperature'], semaphore, run_config['max_tokens'], run_config['max_retries'], run_config
    )
...
async def worker_task(client: openai.AsyncOpenAI, semaphore: asyncio.Semaphore, run_config: dict, queue: asyncio.Queue, results: list, sphere_rows: list, sphere_embeddings: np.ndarray, valid_names_exact: set, valid_names_norm: dict):
    ...
    while not stop_event_async.is_set():
        ...
        if func_embedding is None:
            ...
            if progress_callback_async: progress_callback_async('classify', 1)
            queue.task_done()
            continue

        similarities = np.dot(sphere_embeddings, func_embedding) / (np.linalg.norm(sphere_embeddings, axis=1) * np.linalg.norm(func_embedding))
        ...
        for i in range(0, len(top_candidate_indices), batch_size):
            ...
            if not len(batch_indices): break
            ...
            batch_res = await classify_one_async(client, semaphore, run_config, func_item, spheres_prompt_short, valid_names_exact, valid_names_norm)
...
def add_hierarchy_spheres(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_copy['Sphere_2'] = df_copy['Sphere_2_code'].map(SPHERES_MAP).fillna('').astype(str).apply(lambda x: x.capitalize())
    df_copy['Sphere_3'] = df_copy['Sphere_3_code'].map(SPHERES_MAP).fillna('').astype(str).apply(lambda x: x.capitalize())

    df_copy['Sphere_2'] = df_copy.apply(lambda row: f"{row['Sphere_2_code']} {row['Sphere_2']}" if row['Sphere_2'] else '', axis=1)
    df_copy['Sphere_3'] = df_copy.apply(lambda row: f"{row['Sphere_3_code']} {row['Sphere_3']}" if row['Sphere_3'] else '', axis=1)
...
class SphereClassifierApp:
    ...
    def _build_ui(self):
        ...
        self.spheres_path_var = tk.StringVar(); self.functions_path_var = tk.StringVar(); self.output_path_var = tk.StringVar()
        self._create_file_row(files_frame, "Файл со сферами:", self.spheres_path_var, self._choose_spheres_file)
        self._create_file_row(files_frame, "Файл с функциями:", self.functions_path_var, self._choose_functions_file)
        self._create_file_row(files_frame, "Итоговый файл:", self.output_path_var, self._choose_output_file)
        ...
        # --- Общие настройки ---
        common_settings_frame = ttk.LabelFrame(left_panel, text="Этап 3: Параметры обработки")
        ...
        self._create_entry_row(common_settings_frame, "Сервер эмбеддингов:", self.embed_server_var)
        self._create_entry_row(common_settings_frame, "Модель эмбеддингов:", self.embed_model_var)
        self._create_entry_row(common_settings_frame, "Кандидатов для LLM (Top-K):", self.top_k_var)
        self._create_entry_row(common_settings_frame, "Размер порции для LLM:", self.batch_size_var)
        self._create_entry_row(common_settings_frame, "Макс. попыток API:", self.retries_var)
        self._create_entry_row(common_settings_frame, "Температура (0.0-2.0):", self.temp_var)
        self._create_entry_row(common_settings_frame, "Макс. токенов:", self.max_tokens_var)
        ...
        all_bars = [('embed_spheres', 'Векторизация Сфер'), ('embed_funcs', 'Векторизация Функций'), ('classify', 'Классификация')]
        ...
        self.system_prompt_text = self._create_prompt_tab(prompts_pane, "Системный промпт", SYSTEM_PROMPT_TEMPLATE)
    ...
    def _create_prompt_tab(self, notebook, title, content):
        ...
        text = tk.Text(frame, wrap=tk.WORD, height=10); text.pack(fill=tk.BOTH, expand=True)
    ...
    def _choose_spheres_file(self): self._choose_file(self.spheres_path_var, "Выберите файл со сферами")
    def _choose_functions_file(self): self._choose_file(self.functions_path_var, "Выберите файл с функциями")
    def _choose_output_file(self): self._choose_file(self.output_path_var, "Укажите итоговый файл", is_save=True)
    ...
    def _on_start(self):
        ...
        try:
            config = {
                'spheres_path': self.spheres_path_var.get(), 'functions_path': self.functions_path_var.get(), 'output_path': self.output_path_var.get(),
                'ai_mode': self.ai_mode_var.get(), 'system_prompt': self.system_prompt_text.get("1.0", tk.END),
                'embed_server': self.embed_server_var.get(), 'embed_model': self.embed_model_var.get(),
                'top_k_filter': self.top_k_var.get(), 'classification_batch_size': self.batch_size_var.get(),
                'max_retries': self.retries_var.get(), 'temperature': self.temp_var.get(), 'max_tokens': self.max_tokens_var.get(),
            }

            if config['ai_mode'] == 'Онлайн':
                if not self.api_key_var.get(): raise ValueError("В режиме 'Онлайн' нужен OpenAI API Key.")
            ...
            if not all([config['spheres_path'], config['functions_path'], config['output_path']]):
                raise ValueError("Необходимо указать все три пути к файлам.")
            if config['concurrent_requests'] <= 0 or config['top_k_filter'] <= 0 or config['classification_batch_size'] <= 0:
                raise ValueError("Параметры параллельных запросов, Top-K и размер порции должны быть > 0.")
        except (ValueError, tk.TclError) as e:
            messagebox.showerror("Ошибка валидации", str(e)); return
        ...
        self._worker_thread = Thread(target=self._worker_main, args=(config,), daemon=True)
    ...
    async def _analysis_logic(self, config: dict):
        ...
        try:
            ...
            require_cols(spheres_df, [SPHERE_NAME_COL, SPHERE_DESC_COL, SPHERE_ACTIVITIES_COL], "Сферы")
            require_cols(functions_df, [FUNC_GO_COL, FUNC_TEXT_COL], "Функции")

            if FUNC_ID_COL not in functions_df.columns:
                self.log(f"Колонка '{FUNC_ID_COL}' не найдена. Создаю ее на основе индекса.", to_terminal=True)
            ...
            if 'Sphere_3' in functions_df.columns:
                ...
                if not df_already_processed.empty:
                    self.log(f"Найдено {len(df_already_processed)} функций с уже заполненной 'Sphere_3'. Они будут пропущены.", to_terminal=True)
            else:
                self.log("Колонка 'Sphere_3' не найдена. Все функции будут обработаны.", to_terminal=True)
                df_to_process = functions_df.copy()

            if df_to_process.empty:
                self.log("Не найдено новых функций для обработки. Процесс завершен.", to_terminal=True)
            ...
            if config['ai_mode'] == 'Онлайн':
                ...
                self.log(f"Режим: Локальный. Серверов: {num_servers}. Запросов на сервер: {concurrency_per_server}", to_terminal=True)
            ...
            self.log(f"Получение эмбеддингов для {len(sphere_rows)} Сфер...", to_terminal=True)
            progress_callback_async('reset', ('embed_spheres', len(sphere_rows)))
            sphere_corpus = [f"Сфера: {r.get(SPHERE_NAME_COL, '')}. Описание: {r.get(SPHERE_DESC_COL, '')}. Деятельность: {r.get(SPHERE_ACTIVITIES_COL, '')}" for r in sphere_rows]
            sphere_embeddings_list = []
            for i in range(0, len(sphere_corpus), 64):
                if stop_event_async.is_set(): return
                batch_texts = sphere_corpus[i:i+64]
                batch_embeds = await async_get_embedding_batch(embed_client, config['embed_model'], batch_texts, embed_semaphore)
                if batch_embeds: sphere_embeddings_list.extend(batch_embeds)
                progress_callback_async('embed_spheres', len(batch_texts))
            sphere_embeddings = np.array(sphere_embeddings_list)

            self.log(f"Получение эмбеддингов для {len(function_rows)} Функций...", to_terminal=True)
            progress_callback_async('reset', ('embed_funcs', len(function_rows)))
            func_corpus = [f"Орган: {r.get(FUNC_GO_COL, '')}. Функция: {r.get(FUNC_TEXT_COL, '')}" for r in function_rows]
            function_embeddings = []
            for i in range(0, len(func_corpus), 64):
                if stop_event_async.is_set(): return
                batch_texts = func_corpus[i:i+64]
                batch_embeds = await async_get_embedding_batch(embed_client, config['embed_model'], batch_texts, embed_semaphore)
                if batch_embeds: function_embeddings.extend(batch_embeds)
                else: function_embeddings.extend([None] * len(batch_texts))
                progress_callback_async('embed_funcs', len(batch_texts))

            await embed_client.close()
            self.log("Векторизация завершена.", to_terminal=True)

            self.log(f"Начинаем классификацию {len(function_rows)} функций...", to_terminal=True)
            progress_callback_async('reset', ('classify', len(function_rows)))

            q = asyncio.Queue()
            for i, item in enumerate(function_rows): q.put_nowait((i, item, function_embeddings[i]))

            results = [None] * len(function_rows)

            tasks = [
                asyncio.create_task(worker_task(
                    next(client_cycle), semaphore, config, q, results, sphere_rows, sphere_embeddings, set(sphere_exact.keys()), sphere_norm
            ...
            if stop_event_async.is_set(): self.log("Процесс остановлен.", to_terminal=True); return
            ...
            for i, func_row in enumerate(function_rows):
                ...
                if result_item and result_item.get("status") == "OK": sphere_name = result_item.get("assigned_name", "ERROR")
                elif result_item: sphere_name = result_item.get("status", "UNKNOWN_ERROR")
                new_row = func_row.copy(); new_row["Sphere"] = sphere_name; output_rows.append(new_row)
            ...
            self.log("ШАГ 2: ДОБАВЛЕНИЕ ИЕРАРХИЧЕСКИХ СФЕР (Sphere_2, Sphere_3)", to_terminal=True)
            ...
            if FUNC_ID_COL in functions_df.columns and FUNC_ID_COL in final_df.columns:
                try:
                    ...
                    final_df.drop_duplicates(subset=[FUNC_ID_COL], keep='last', inplace=True)
                    final_df = final_df.set_index(FUNC_ID_COL).loc[functions_df[FUNC_ID_COL]].reset_index()
                except KeyError:
                    self.log("Не удалось полностью восстановить исходный порядок строк. Результат может быть отсортирован иначе.", to_terminal=True)
            # --- КОНЕЦ ИЗМЕНЕНИЯ ---

            self.log(f"Сохранение итогового результата в файл '{config['output_path']}'...", to_terminal=True)
