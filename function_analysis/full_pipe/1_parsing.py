...
ABBR_STOPWORDS = {
    'и', 'по', 'о', 'в', 'на', 'об', 'с', 'при', 'для', 'над', 'под', 'из', 'во', 'со',
    'республика', 'республики', 'казахстан', 'казахстана', 'государственного', 'учреждения'
}
...
def fast_clean_and_format_text(text: str) -> str:
    ...
    patterns_to_remove = [
        r'\s*сноска\s*\..*', r'\s*примечание\s*ИЗПИ!.*', r'\s*\(?вводится в действие.*?(\(|$)',
        r'\(порядок введения в действие см\. п\. \d+\)', r'\s*искл[ю]?че?н[оа]?.*',
        r'\s*действовал[аи]? до \d{2}\.\d{2}\.\d{4}.*', r';\s*от\s+\d{2}\.\d{2}\.\d{4}.*',
        r'\s*см\. п\. \d+',
    ]
...
def create_abbreviation(name: str) -> str:
    ...
    if not isinstance(name, str) or not name.strip(): return "GO"
    name = " ".join(name.split())
    tokens = re.split(r'[\s\-]+', name)
    if all(tok.isupper() and len(tok) > 1 for tok in tokens): return tokens[0]
...
class ParserApp:
    ...
    def build_ui(self):
        ...
        self._create_folder_row(files_frame, "Папка с документами:", self.input_folder_var, self._choose_input_folder)
        ...
        perf_frame = ttk.LabelFrame(left_panel, text="Этап 3: Настройки производительности")
        ...
        self.start_btn = ttk.Button(controls_frame, text="Старт анализа", command=self._on_start, style="Accent.TButton")
        self.start_btn.pack(side=tk.LEFT, expand=True, fill=tk.X, ipady=5, padx=5, pady=5)
        self.stop_btn = ttk.Button(controls_frame, text="Стоп", command=self._on_stop, state=tk.DISABLED)
    ...
    def _create_ai_mode_selector(self, parent):
        ...
        ttk.Radiobutton(mode_frame, text="Онлайн", variable=self.ai_mode_var, value="Онлайн",
                        command=self._on_ai_mode_change).pack(side=tk.LEFT)
        ttk.Radiobutton(mode_frame, text="Локальный", variable=self.ai_mode_var, value="Локальный",
                        command=self._on_ai_mode_change).pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(mode_frame, text="АП", variable=self.ai_mode_var, value="АП",
                        command=self._on_ai_mode_change).pack(side=tk.LEFT)
    ...
    def _create_ai_config_frames(self, parent):
        ...
        row.pack(fill=tk.X, pady=3, padx=5)
        ttk.Label(row, text="URL серверов (1 на строку):", width=25).pack(side=tk.LEFT, anchor='n')
    ...
    def _choose_input_folder(self):
        ...
        if d: self.input_folder_var.set(d)
    ...
    def _on_start(self):
        ...
        self._worker_thread = threading.Thread(target=self._master_thread_logic, args=(config,), daemon=True)
    ...
    def _master_thread_logic(self, config: dict):
        try:
            ...
            log_to_terminal(f"Найдено {total_files} файлов ({len(docx_files)} docx, {len(doc_files)} doc).")

            # Определяем список серверов для работы
            is_local_multi = config['ai_mode'] == 'Локальный' and 'base_urls' in config
            servers = config['base_urls'] if is_local_multi else [config.get('base_url')]
            ...
            if docx_files:
                ...
                log_to_terminal(f"Распределение воркеров по серверам: {list(zip(servers, workers_per_server))}")
                ...
                for server_url, worker_count in zip(servers, workers_per_server):
                    ...
                    log_to_terminal(f"Запуск {worker_count} воркеров для сервера {server_url}...")
                    for _ in range(worker_count):
                        thread = threading.Thread(target=self._file_processor_worker,
            ...
            if self._stop_flag.is_set(): raise InterruptedError()
            ...
            if doc_files:
                ...
                ai_client = OpenAI(base_url=doc_server_url, api_key=config.get('api_key', 'not-needed'),
    ...
    def _process_single_file(self, filepath, results_queue, config, ai_client):
        ...
        try:
            ...
            full_go_name = self._extract_full_go_name(paragraphs, filename, config, ai_client)
            if not full_go_name:
                with self.stats_lock: self.stats['name_extracted_fail'] += 1
                log_to_terminal(f"⚠️ Пропуск {filename}: не удалось извлечь полное название ГО.")
                return
            with self.stats_lock: self.stats['name_extracted_success'] += 1
            ...
            if status != 'OK':
                if status == 'HEADER_NOT_FOUND':
                    with self.stats_lock: self.stats['functions_header_not_found'] += 1
                    log_to_terminal(f"⚠️ {filename}: Пропуск. Не найден заголовок раздела 'Функции'.")
                elif status == 'NO_ITEMS_FOUND':
                    with self.stats_lock: self.stats['functions_found_empty'] += 1
                    log_to_terminal(f"⚠️ {filename}: Пропуск. Раздел 'Функции' найден, но он пуст.")
                return
            with self.stats_lock: self.stats['functions_found_success'] += 1
    ...
    def _file_processor_worker(self, tasks_queue: queue.Queue, results_queue: queue.Queue, config: dict,
                               base_url: str):
        ...
        ai_client = OpenAI(base_url=base_url, api_key=config.get('api_key', 'not-needed'),
                           http_client=httpx.Client(timeout=config.get('timeout', 60.0)))
    ...
    def _extract_full_go_name(paragraphs, filename, config, client):
        ...
        if not full_text.strip():
            log_to_terminal(f"❌ Пропуск {filename}: не найден текст для анализа в первых 150 абзацах.")
            return None
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt + 1 == max_retries:
                    log_to_terminal(f"  [AI] ❌ Превышено количество попыток для {filename}.")
                    return None
                time.sleep(delay)
                delay *= 2
            except Exception as e:
                log_to_terminal(f"  [AI] ❌ Непредвиденная ошибка на этапе извлечения ГО для {filename}: {e}")
                traceback.print_exc()
                return None
        return None
